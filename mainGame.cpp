...
int main(int argc, char* argv[])
{
    ...
    while(gameRunning)
    {
        ...
        if((plr1.charStat == win && (SDL_GetTicks() - plr1.waitTimeToPose >= 4000)) || (plr2.charStat == win && (SDL_GetTicks() - plr2.waitTimeToPose >= 4000))){
            SDL_RenderClear(renderer);
            SDL_RenderPresent(renderer);
            SDL_Delay(750);
            plr1.initplayer1(background);
